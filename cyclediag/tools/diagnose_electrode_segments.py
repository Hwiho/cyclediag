...
def _build_report(cell_id: str, feats: pd.DataFrame, segs: pd.DataFrame, lib_meta: dict) -> str:
    lines = [
        f"# 열화 구간별 전극 가설 진단 v1.1 — {cell_id}",
        "",
        f"- Level: **hypothesis_bol_ocp** (aged 하프셀 교정 아님)",
        f"- Methodology: electrode_side_v1_1 · FC-OCP peak Δhits · contact_stack vs NE(Si co-sign)",
        f"- OCP library: anode={lib_meta.get('n_anode_curves')} cathode={lib_meta.get('n_cathode_curves')} aged={lib_meta.get('aged_data')}",
        f"- 분석 포인트: {len(feats)} cycles sampled; 세그먼트 SoHQ≥50",
        "",
        "## 구간 요약",
        "",
    ]
    ...
    lines.append("## 사이클 궤적")
    lines.append("")
    lines.append("| cycle | SoHQ | dominant | PE | contact | NE_hyp | LAM_PE | contact_loss | note |")
    lines.append("|------:|-----:|:---------|---:|--------:|-------:|-------:|-------------:|:-----|")
    for _, r in show.iterrows():
        note = str(r.get("electrode_narrative") or "")[:48].replace("|", "/")
        cl = r.get("contact_stack_score")
        if cl is None or (isinstance(cl, float) and np.isnan(cl)):
            cl = r.get("contact_loss_score") or 0
        lines.append(
            f"| {int(r['cycle'])} | {float(r.get('SoHQ') or 0):.1f} | {r.get('dominant_electrode')} | "
    ...
    lines.append("")
    lines.append(
        "> v1.1: `contact_stack` = 전극 미분해 접촉/스택 저항 패턴. "
        "`NE`는 Si co-sign(hyst_low·mech/chem·Q_relax)이 있을 때만. "
        "절대 LAM% 금지. peak attribution은 synth FC-OCP 도메인."
    )
    return "\n".join(lines)
...
def main() -> None:
    ...
    keep = [c for c in [
        ...
        "eta_argmax_SOC", "mech_vs_chem_ratio", "PER", "LAM_curve_proxy",
