...
### 하락 트렌드 (상위)
- LAM 곡선 proxy: early=-3.194 → late=-16.44 (Δ=-13.24, -5.251%/100cyc) → decreasing · context
- LLI 곡선 proxy: early=-0.5851 → late=-2.774 (Δ=-2.189, -1.046%/100cyc) → decreasing · context
- 방전 플래토 폭: early=17.32 → late=15.3 (Δ=-2.017, -0.9287Q-units/100cyc) → decreasing · matches_aging
- RΩ 성장률 /100cyc: early=0.7607 → late=0.4498 (Δ=-0.3109, -0.3132mΩ/100cyc/100cyc) → decreasing · context
- 방전 플래토 ΔV: early=-0.0394 → late=-0.08082 (Δ=-0.04142, -0.01907V/100cyc) → decreasing · context

### 상승 트렌드 (상위)
- 옴 저항 (SOC50): early=1.349 → late=2.293 (Δ=+0.9446, +0.4789mΩ/100cyc) → increasing · matches_aging
- 기계/화학 비: early=1.86 → late=2.422 (Δ=+0.5618, +0.33311/100cyc) → increasing · matches_aging
- 완화 용량 회복: early=-0.5043 → late=-0.08267 (Δ=+0.4216, +0.247%/100cyc) → increasing · matches_aging
- 계면 R 패턴: early=0.4692 → late=0.8449 (Δ=+0.3757, +0.15480–1/100cyc) → increasing · matches_aging
- PE activity 패턴: early=0.6565 → late=0.9319 (Δ=+0.2754, +0.1380–1/100cyc) → increasing · matches_aging

### 기대 aging과 반대
- 저SOC 히스테리시스: early=0.07667 → late=0.04561 (Δ=-0.03106, -0.01231V/100cyc) → decreasing · opposite_aging
...
- 트렌드: 완화 용량 회복: early=-0.5043 → late=-0.08267 (Δ=+0.4216, +0.247%/100cyc) → increasing · matches_aging
...
- 트렌드: 옴 저항 (SOC50): early=1.349 → late=2.293 (Δ=+0.9446, +0.4789mΩ/100cyc) → increasing · matches_aging

**전하전달 저항 (SOC50)** (`R_ct_soc50`)
- 의미: 중간 시간대 잔차의 지수 포화항. 계면/화학 저항 proxy (Cdl 직접 추정 아님).
- 계산: resid = R−RΩ−A√t; t∈[0.3,10]s 에서 Rct(1−e^(−t/τ)) fit.
- 트렌드: 전하전달 저항 (SOC50): early=0.7252 → late=0.947 (Δ=+0.2218, +0.09667mΩ/100cyc) → increasing · matches_aging
...
- 트렌드: RΩ 성장률 /100cyc: early=0.7607 → late=0.4498 (Δ=-0.3109, -0.3132mΩ/100cyc/100cyc) → decreasing · context
...
- 트렌드: Rate capability factor: early=0.9687 → late=0.9519 (Δ=-0.01679, -0.0012021/100cyc) → flat · stable
...
- 트렌드: 접촉/스택 손실: early=0.8053 → late=0.9259 (Δ=+0.1206, +0.074440–1/100cyc) → increasing · matches_aging
...
- 계산: A_diff↑, PER↑, RCF↓ 등.
- 트렌드: 고체확산 제한: early=0.555 → late=0.7449 (Δ=+0.1899, +0.036970–1/100cyc) → flat · stable
...
- 트렌드: PE lean 점수: early=0.5423 → late=0.7989 (Δ=+0.2566, +0.11390–1/100cyc) → increasing · matches_aging
...
- 트렌드: contact_stack lean: early=0.8053 → late=0.9259 (Δ=+0.1206, +0.074440–1/100cyc) → increasing · matches_aging

**NE 가설 점수** (`NE_side_score`)
- 의미: contact × Si chemo-mech co-sign. Si 없으면 NE 확정 금지.
- 계산: 0.55·contact·(0.25+0.75·si) + 0.30·NE_boost·si.
- 트렌드: NE 가설 점수: early=0.1772 → late=0.2804 (Δ=+0.1032, +0.055850–1/100cyc) → increasing · matches_aging
