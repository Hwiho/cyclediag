...
- 트렌드: 완화 용량 회복: early=-0.4453 → late=0.01934 (Δ=+0.4647, +0.2544%/100cyc) → increasing · matches_aging
...
- 트렌드: 옴 저항 (SOC50): early=1.092 → late=2.744 (Δ=+1.651, +0.7975mΩ/100cyc) → increasing · matches_aging

**전하전달 저항 (SOC50)** (`R_ct_soc50`)
- 의미: 중간 시간대 잔차의 지수 포화항. 계면/화학 저항 proxy (Cdl 직접 추정 아님).
- 계산: resid = R−RΩ−A√t; t∈[0.3,10]s 에서 Rct(1−e^(−t/τ)) fit.
- 트렌드: 전하전달 저항 (SOC50): early=0.7379 → late=0.9016 (Δ=+0.1637, +0.06968mΩ/100cyc) → increasing · matches_aging
...
- 트렌드: RΩ 성장률 /100cyc: early=1.104 → late=0.7864 (Δ=-0.3172, -0.3213mΩ/100cyc/100cyc) → decreasing · context
...
- 트렌드: Rate capability factor: early=0.9772 → late=0.9672 (Δ=-0.009985, +0.0016941/100cyc) → flat · stable
...
- 트렌드: 접촉/스택 손실: early=0.7447 → late=0.9905 (Δ=+0.2458, +0.15760–1/100cyc) → increasing · matches_aging
...
- 계산: A_diff↑, PER↑, RCF↓ 등.
- 트렌드: 고체확산 제한: early=0.4264 → late=0.5751 (Δ=+0.1488, -0.0010570–1/100cyc) → flat · stable
...
- 트렌드: contact_stack lean: early=0.7447 → late=0.9905 (Δ=+0.2458, +0.15760–1/100cyc) → increasing · matches_aging
